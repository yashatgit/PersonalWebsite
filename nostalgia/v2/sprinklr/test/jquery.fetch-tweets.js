...
(function ($) {
  ...
  var twitterify = (function () {
    var r = [
      ...
      [
        /@(\w+)/g,
        '<a class="tweet-mention" href="https://twitter.com/$1">@$1</a>',
      ],
      [
        /#(\w+)/g,
        '<a class="tweet-hash" href="http://search.twitter.com/search?q=$1">#$1</a>',
      ],
    ];
    var le = r.length;
  ...
  var createStatusUpdatesFetcher = function (obj, username, count) {
        ...
        for (i = 0; i < le; i++) {
          ...
          tweetHtml.push(
            [
              ...
              twitterify(current.text),
              "</div>",
              '<div class="tweet-created_at"><a href="http://www.twitter.com/',
              username,
              "/status/",
              current.id_str,
              '">',
  ...
  var fetchUserData = function (obj, username) {
    var url =
      "http://api.twitter.com/1/users/show.json?callback=?&screen_name=";
    var userInfoDiv = $('<div class="tweets-userinfo">').appendTo(obj);

    userInfoDiv.html(
      '<div class="tweets-loading">Loading profile data...</div>'
    );
    ...
    $.getJSON(url + username, function (data) {
      ...
      userInfoHtml +=
        ...
        "<h4>" +
        data.name +
        '<span class="tweets-userinfo-username">(<a href="' +
        "https://twitter.com/" +
        username +
        '">@' +
        username +
  ...
  $.fn.fetchTweets = function (options) {
    ...
    fetchStatusUpdates = createStatusUpdatesFetcher(obj, username, count);
